...
document.writeln("#divM {width: 550px;height: 326px;background:url(/swt/swtimg/gtswtbg11.png) 0px 0 no-repeat; left: 50%;bottom: 50%;margin-left: -195px;margin-bottom: -115px;_position: absolute;_bottom: expression(offsetParent.scrollTop+242);box-shadow: 0 0 35px #000;-webkit-box-shadow: 0 0 35px #000;-moz-box-shadow: 0 0 35px #000;}");
...
document.writeln("#divMz{background: url(/swt/swtimg/swtanniu.png) no-repeat; width:95px; height:35px; display:block; overflow:hidden;position: absolute;top: 270px;left: 320px;}");

document.writeln("#divMx{background: url(/swt/swtimg/swtanniu.png) -104px 0 no-repeat; width:95px; height:35px; display:block; overflow:hidden;position: absolute;top: 270px;left: 430px;}");

document.writeln("#divM #divMa1{left:320px}");

document.writeln("#divM #divMa2{left:430px}");
...
document.writeln("     <a id=\"divMa2\" href=\"javascript:void(0)\" onclick=\"closeM()\" target=\"_self\" title=\"稍后联系\"></a>");
...
//如果用户不点击就自动化
	...
	$("#divMagb,#divMa2").click(function(){
		...
		setTimeout("openMdivM()",15000);//15秒后再次弹出

		setTimeout("openMdivM_suoxiao1()",30000);//中间点击3秒后加载亮背景class

		setTimeout("openMdivM_suoxiao2()",50000);//中间点击5秒后移除亮背景class

		

		$("#divM").animate({'width':'0','height': '0','left': '0%','bottom': '0%','margin-left':'0px','margin-bottom': '0px'},1000);

		$("#divM").fadeOut(1000).delay(5000).animate({'width':'550px','height': '326px','left': '50%','bottom': '50%','margin-left':'-195px','margin-bottom': '-115px'},1000);
